class UserServices {
  ...
  signUp = async (
    ...
    try {
      ...
      const department = await this.repo.userRepo.getDeptByName(
        department_name
      );
      ...
      let existingUser = await this.repo.userRepo.getUserByEmailAndUserCode(
        email,
        userCode
      );
  ...
  getFilteredUsersByReqToAndFrom = async (user, userFriends) => {
    ...
    return userFriends.map((friend) => {
      ...
      receiverUser =
        user && friend.req_from.id == user.user_id
          ? friend.req_to
          : friend.req_from;
